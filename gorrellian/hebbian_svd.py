...
def svd_solve(gen_pair, L=5, eta=1e-5, C_barrier=0.1, verbose=False):
    """
    ...
    N = len(a)
    M = len(b)
    # guess for number of iterations required
    ...
    while C_last > C or C > C_barrier:
        ...
        if isnan(UV['U']).any() or isnan(UV['V']).any():
            U, V = U0, V0
            eta = eta / 2
            print(f"eta too large, resetting, reducing to {eta}.  ")
        else:
            U0, V0 = U, V
        if verbose:
            # U'U and V'V should approach I with convergence; print the diagonals.
            print(f" {iter:08d} / {iterations:08d}  C: {C:05f}  eta: {eta}    ", end='\r')
